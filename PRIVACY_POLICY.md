# Privacy Policy

**Effective Date:** January 1, 2025  
**Last Updated:** January 1, 2025

---

## 1. Introduction

Welcome to **DoubleMyHolidays** ("we," "our," or "us"). We respect your privacy and are committed to protecting your personal data. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our vacation optimization service available at [your-domain.com] (the "Service").

This policy complies with the **General Data Protection Regulation (GDPR)**, the **California Consumer Privacy Act (CCPA)**, and other applicable data protection laws.

**By using our Service, you agree to the collection and use of information in accordance with this Privacy Policy.**

---

## 2. Information We Collect

### 2.1 Information You Provide Directly

When you use DoubleMyHolidays, we collect the following information:

- **Email Address**: Collected during payment checkout to send purchase receipts and communicate about your account
- **Payment Information**: Processed securely through Stripe (see Section 5.3)
- **Country and Region**: Used to determine applicable public holidays and regional pricing
- **Vacation Preferences**: Including PTO days available, preferred vacation strategies, and timeframes

### 2.2 Sensitive Financial Data (NOT STORED)

**IMPORTANT**: Your **salary information** is processed **entirely client-side** in your browser and is **NEVER transmitted to our servers, stored in our database, or accessible to DoubleMyHolidays**. 

This data is used solely for:
- Calculating the monetary value of optimized vacation time
- Displaying personalized recommendations

**Once you close your browser or refresh the page, this information is permanently deleted.**

### 2.3 Automatically Collected Information

We automatically collect certain information when you use our Service:

- **Usage Data**: Pages visited, features used, time spent on the Service
- **Device Information**: Browser type, operating system, IP address, device identifiers
- **Analytics Data**: Collected via PostHog (see Section 5.4) for product improvement
- **Cookies**: See Section 7 for details

### 2.4 Information We Do NOT Collect

We do not collect:
- Social Security Numbers or government identifiers
- Credit card numbers (processed by Stripe, not stored by us)
- Location data beyond country/region selection
- Biometric data
- Personal health information

---

## 3. How We Use Your Information

We use your information for the following purposes:

### 3.1 Service Delivery
- Generate personalized vacation optimization plans
- Calculate vacation efficiency and monetary value
- Provide country-specific holiday calendars

### 3.2 Payment Processing
- Process one-time payments via Stripe
- Send payment receipts to your email
- Manage transaction disputes or refunds

### 3.3 Communications
- Send transactional emails (purchase confirmations, receipts)
- Respond to customer support inquiries
- Send service updates (with your consent)

### 3.4 Analytics & Improvement
- Analyze usage patterns to improve the Service
- Monitor performance and fix bugs
- Conduct A/B testing for feature optimization

### 3.5 Legal Compliance
- Comply with legal obligations
- Enforce our Terms of Service
- Protect against fraud and abuse

**We do NOT sell your personal information to third parties.**

---

## 4. Legal Basis for Processing (GDPR)

Under GDPR, we process your personal data based on the following legal grounds:

| Purpose | Legal Basis |
|---------|-------------|
| Service delivery | **Contract** - Necessary to provide the Service you requested |
| Payment processing | **Contract** - Necessary to complete your purchase |
| Analytics & improvement | **Legitimate Interest** - To improve our Service |
| Marketing emails | **Consent** - Only with your explicit opt-in |
| Legal compliance | **Legal Obligation** - Required by law |

You have the right to object to processing based on legitimate interest (see Section 9).

---

## 5. Third-Party Service Providers

We share your information with the following trusted third parties:

### 5.1 Vercel (Hosting)

**Service**: Web hosting and infrastructure  
**Data Shared**: Usage data, IP addresses, device information  
**Purpose**: Deliver the Service globally via CDN  
**Privacy Policy**: https://vercel.com/legal/privacy-policy  
**Location**: United States (EU-US Data Privacy Framework compliant)

### 5.2 Stripe (Payment Processing)

**Service**: Payment processing  
**Data Shared**: Email, payment information, transaction metadata  
**Purpose**: Securely process payments and manage transactions  
**Privacy Policy**: https://stripe.com/privacy  
**Location**: United States (PCI DSS Level 1 certified)  
**Note**: Credit card data is handled entirely by Stripe and never touches our servers

### 5.3 PostHog (Analytics)

**Service**: Product analytics  
**Data Shared**: Usage patterns, feature interactions, anonymized user behavior  
**Purpose**: Improve product features and user experience  
**Privacy Policy**: https://posthog.com/privacy  
**Location**: United States (self-hosted option available)  
**Data Retention**: 90 days (configurable)

**Note**: We use privacy-friendly analytics with IP anonymization enabled.

### 5.4 Data Processing Agreements

We have executed Data Processing Agreements (DPAs) with all third-party processors to ensure GDPR compliance.

---

## 6. Data Security

We implement industry-standard security measures to protect your data:

### 6.1 Technical Safeguards
- ✅ **HTTPS/TLS Encryption**: All data in transit is encrypted
- ✅ **Content Security Policy (CSP)**: Prevents XSS attacks
- ✅ **Security Headers**: X-Frame-Options, X-Content-Type-Options
- ✅ **Secure Payment Processing**: PCI DSS compliant via Stripe
- ✅ **No Server-Side Salary Storage**: Sensitive financial data never leaves your device

### 6.2 Organizational Safeguards
- Limited employee access to personal data
- Regular security audits and penetration testing
- Incident response procedures
- Employee confidentiality agreements

**No method of transmission over the Internet is 100% secure. While we strive to protect your data, we cannot guarantee absolute security.**

---

## 7. Cookies and Tracking Technologies

### 7.1 Cookies We Use

| Cookie Type | Purpose | Duration |
|-------------|---------|----------|
| **Essential Cookies** | Session management, authentication | Session |
| **Analytics Cookies** | PostHog analytics (optional) | 90 days |
| **Preference Cookies** | Remember your settings | 1 year |

### 7.2 Third-Party Cookies

- **Stripe**: Payment processing and fraud prevention
- **PostHog**: Analytics and usage tracking
- **Vercel**: Performance monitoring

### 7.3 Managing Cookies

You can control cookies through your browser settings:
- **Chrome**: Settings > Privacy and Security > Cookies
- **Firefox**: Preferences > Privacy & Security > Cookies
- **Safari**: Preferences > Privacy > Cookies

**Note**: Disabling essential cookies may limit Service functionality.

---

## 8. Data Retention

We retain your data only as long as necessary:

| Data Type | Retention Period | Reason |
|-----------|------------------|--------|
| **Email & Transaction Data** | 7 years | Tax and accounting requirements |
| **Vacation Plans** | Not stored | Generated client-side only |
| **Salary Information** | 0 days | Never stored (client-side only) |
| **Analytics Data** | 90 days | Product improvement |
| **Support Tickets** | 3 years | Customer service records |

After the retention period, data is securely deleted or anonymized.

---

## 9. Your Privacy Rights

### 9.1 GDPR Rights (EU/UK/EEA Residents)

You have the following rights under GDPR:

1. **Right to Access**: Request a copy of your personal data
2. **Right to Rectification**: Correct inaccurate data
3. **Right to Erasure**: Request deletion of your data ("right to be forgotten")
4. **Right to Restrict Processing**: Limit how we use your data
5. **Right to Data Portability**: Receive your data in a machine-readable format
6. **Right to Object**: Object to processing based on legitimate interest
7. **Right to Withdraw Consent**: Withdraw consent at any time (for consent-based processing)
8. **Right to Lodge a Complaint**: File a complaint with your supervisory authority

**To exercise your rights, contact us at: privacy@[your-domain].com**

We will respond within **30 days** (may be extended to 60 days for complex requests).

### 9.2 CCPA Rights (California Residents)

California residents have additional rights:

1. **Right to Know**: What personal information we collect and how we use it
2. **Right to Delete**: Request deletion of your personal information
3. **Right to Opt-Out**: Opt-out of the "sale" of personal information (we do NOT sell data)
4. **Right to Non-Discrimination**: Equal service regardless of privacy choices

**To exercise CCPA rights, email: privacy@[your-domain].com or call [your-phone-number]**

### 9.3 How to Exercise Your Rights

**Email**: privacy@[your-domain].com  
**Subject Line**: "Data Privacy Request - [Your Name]"  
**Include**: Description of your request and any relevant account information

We may request additional information to verify your identity before processing requests.

---

## 10. International Data Transfers

DoubleMyHolidays is operated from the **United States**. If you access our Service from outside the US, your data may be transferred to, stored, and processed in the United States.

### 10.1 EU Data Transfers

For users in the European Economic Area (EEA), UK, or Switzerland:

- **Legal Mechanism**: EU-US Data Privacy Framework and Standard Contractual Clauses (SCCs)
- **Safeguards**: All third-party processors comply with GDPR requirements
- **Your Rights**: You retain all GDPR rights regardless of data location

By using our Service, you consent to the transfer of your data to the United States.

---

## 11. Children's Privacy

DoubleMyHolidays is **not intended for children under 16 years of age**. We do not knowingly collect personal information from children.

If you are a parent or guardian and believe your child has provided us with personal data, please contact us immediately at privacy@[your-domain].com. We will delete such information promptly.

---

## 12. Marketing Communications

### 12.1 Opt-In

We will only send marketing emails if you:
- Explicitly opt-in during account creation
- Subscribe to our newsletter
- Consent via a clear affirmative action

### 12.2 Opt-Out

You can unsubscribe at any time by:
- Clicking "Unsubscribe" in any marketing email
- Emailing privacy@[your-domain].com
- Managing preferences in your account settings

**Transactional emails (receipts, service updates) cannot be opted out of.**

---

## 13. Changes to This Privacy Policy

We may update this Privacy Policy from time to time. Changes will be posted on this page with an updated "Last Updated" date.

### 13.1 Notification of Changes

For **material changes**, we will:
- Send an email notification to registered users
- Display a prominent notice on our website
- Request renewed consent where required by law

### 13.2 Your Continued Use

Continued use of the Service after changes constitutes acceptance of the updated Privacy Policy.

**We recommend reviewing this policy periodically.**

---

## 14. Data Breach Notification

In the event of a data breach that affects your personal information:

1. We will notify you **within 72 hours** (as required by GDPR)
2. Notification will include:
   - Nature of the breach
   - Data affected
   - Measures taken to mitigate harm
   - Steps you can take to protect yourself
3. We will notify relevant supervisory authorities as required by law

---

## 15. Contact Information

### 15.1 Data Controller

**DoubleMyHolidays**  
[Your Company Legal Name]  
[Your Business Address]  
Email: privacy@[your-domain].com  
Website: [your-domain.com]

### 15.2 EU Representative (if applicable)

If required under GDPR Article 27:  
[EU Representative Name]  
[EU Representative Address]  
Email: eu-representative@[your-domain].com

### 15.3 Data Protection Officer (DPO)

Email: dpo@[your-domain].com

### 15.4 Supervisory Authority

For EU residents, you have the right to lodge a complaint with your national data protection authority:
- **List of EU Data Protection Authorities**: https://edpb.europa.eu/about-edpb/board/members_en

---

## 16. Additional Information for Specific Jurisdictions

### 16.1 California Residents (CCPA/CPRA)

**Categories of Personal Information Collected** (last 12 months):
- Identifiers (email, IP address)
- Commercial information (purchase history)
- Internet activity (usage data)
- Geolocation data (country/region)

**We do NOT sell personal information.**

**Shine the Light Law**: California residents may request information about disclosures to third parties once per year (email: privacy@[your-domain].com).

### 16.2 Nevada Residents

We do not sell personal information. Nevada residents may opt-out of future sales (even though we don't engage in this practice) by emailing privacy@[your-domain].com.

### 16.3 UK Residents (UK GDPR)

Following Brexit, UK GDPR applies to UK residents. Your rights and protections are equivalent to those under EU GDPR.

**UK Supervisory Authority**: Information Commissioner's Office (ICO)  
Website: https://ico.org.uk

---

## 17. Automated Decision-Making

We do **NOT** use automated decision-making or profiling that produces legal effects or significantly affects you.

The vacation optimization algorithm is a **tool** that provides recommendations, but all final decisions are made by you.

---

## 18. Your California "Do Not Track" Rights

DoubleMyHolidays does not currently respond to "Do Not Track" (DNT) browser signals. We honor opt-out requests made directly to us.

Third-party analytics providers (PostHog) may collect data across websites. Refer to their privacy policies for DNT compliance.

---

## 19. Disclaimer

This Privacy Policy is provided for informational purposes. While we strive for accuracy, this policy does not create any contractual rights or obligations beyond those required by law.

For legally binding terms, please review our **Terms of Service**.

---

## 20. Acknowledgment

By using DoubleMyHolidays, you acknowledge that you have read and understood this Privacy Policy and agree to its terms.

If you do not agree with this policy, please discontinue use of the Service immediately.

---

**Last Updated:** January 1, 2025  
**Version:** 1.0

---

© 2025 DoubleMyHolidays. All rights reserved.
